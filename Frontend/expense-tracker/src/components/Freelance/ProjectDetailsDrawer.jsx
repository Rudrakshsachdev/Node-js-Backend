...
export default function ProjectDetailsDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/40 backdrop-blur-sm flex justify-end">
      <div className="bg-white w-full sm:max-w-xl md:max-w-2xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="px-4 sm:px-6 py-4 border-b border-slate-100 flex items-start justify-between gap-3 bg-slate-50/50">
          <div className="space-y-1.5 flex-1 min-w-0 pr-2">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-lg sm:text-xl font-bold text-slate-900 tracking-tight break-words line-clamp-2">
                {project.title}
              </h2>
              <span
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 pt-0.5">
              <span className="flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 shrink-0" /> Received: {project.receivedDate || 'N/A'}
              </span>
              {project.isCompleted && project.completionDate && (
                <span className="flex items-center gap-1 text-emerald-600 font-medium">
                  <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> Completed: {project.completionDate}
                </span>
              )}
            </div>
          </div>
